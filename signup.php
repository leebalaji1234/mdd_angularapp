...
if ($app->is_post()) {
	...
	$_POST['dob'] = $_REQUEST['month']."/".$_REQUEST['day']."/".$_REQUEST['year'];
	 $_REQUEST['dob'] = $_REQUEST['month']."/".$_REQUEST['day']."/".$_REQUEST['year'];
  ...
  else if( !empty($_REQUEST['dob']) && calculateAge($_REQUEST['dob']) < 6)
    $app->error('## Entrant age allowed only 6 and above##');     
    ...
    if (!$user->_new_row)
      ...
      if (isset($_REQUEST['city']) && $_REQUEST['city'] == 'addnew' && !empty($_REQUEST['manualcity'])){
        ...
        if (!$cityexists->_new_row){
          ...
          $user->pass = base64_encode($_REQUEST['pass']);
