// .config(['$interpolateProvider', function ($interpolateProvider) {
 ...
 var md = {};
 window.md = {};
md = angular.module('md', ['ngSanitize','allsocialshare']) //'ui.router'
...
// console.log($scope.searcherAge.ageMax);
		 ...
		 $scope.onDoView = function(did) {
                 $.post('ajax_do_view.php', {
                     drawing_id: did
                 }, function(response) {
                     if (response == 'success') {
                        // alert($('#view1_' + did).text());
                         viewval = parseInt($('#view1_' + did).text());
                         viewval++;
                         // $('#like_' + did).text(likeval);
                         $('#view1_' + did).text(viewval);
                     }
                     // if (response == 'disable') {
                     //     alert('Oops! It seems you already like it');
                     // }
...
var html1 = '<iframe width="660" height="315" src="[placeholder]" frameborder="0" allowfullscreen></iframe>';
...
html1 = html1.replace('[placeholder]',url);
 ...
 $('#expandmk_'+id).parent().find('.makeofwindow').html(html1).slideDown();
		...
		//alert($.type($scope.searcher.country));
		 $scope.listdrawings();
