...
$drawings = $tbl_drawings->find_all("where status=0 ORDER BY proof_file !='' desc ,is_watermark = 0 ,is_sponsored != '' asc,created_at desc");
...
if($drawings){
	...
	foreach ($drawings as $key => $value) {
         ...
         $res =& $db->query("select * from mp_users where id =$value->user_id");
         $userdetail =& $res->fetchRow(DB_FETCHMODE_OBJECT); 
		...
		$carr[$i]['views'] = $value->views?$value->views:0;
