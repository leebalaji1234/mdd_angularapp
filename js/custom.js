...
var refreshId = setInterval(function() {
     ...
     // if($('#slogan1').is(":visible")){
      ...
      // }
      // sleep(3000);
      // if($('#slogan2').is(":visible")){        
      //   $('#slogan2').hide();
      //   $('#slogan1').delay(1000).show();
      // }

}, 5000);
$('.pass_conf_field').on('focusout',function(){
   var currentval = $(this).val();
   setTimeout(function(){
    if(currentval != $('.pass_field').val()){
      alert("Oops ! Passwords not match");
      return false;
    }
   },10);
});
...
function showThemeDesc(tid){
 $.get('ajax_theme_desc.php',{id:tid},function(res){
   if(res == 'none'){
     $('#theme_desc').hide();
   }else{
     $('#theme_desc').show().html(res);
   }
 });
}


function appendTodob(){
var newdate = $('#month').val()+"/"+$('#day').val()+"/"+$('#year').val(); 
$('.dobfield').val(newdate);
}