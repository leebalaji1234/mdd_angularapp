...
// echo "checking".$parent_field_display;exit;
    ...
    }else if(empty($_REQUEST['theme_id'])){
      $app->error('## Please select theme##');
    ...
    if(isset($_FILES) && $_FILES['file']['tmp_name']){
      ...
      file_put_contents($img_name, @$_SESSION['drawing_image']);   
        ...
        $tbl_parent->created_at = Util::epoch_to_datetime();
        $tbl_drawings->updated_at = Util::epoch_to_datetime();
        $tbl_parent->save();
      ...
      $app->mail($_SESSION['email'], 'upload_drawing', array(        
...
$res =& $db->query('SELECT t.* from mp_theme_categories as c INNER JOIN mp_themes as t ON (c.id = t.category_id) where c.type = 2 and t.status = 1 and t.region_id IS NOT NULL
UNION ALL SELECT t.* from mp_theme_categories as c INNER JOIN mp_themes as t ON (c.id = t.category_id) where c.type = 1 and t.status = 1');
 $themes = [];
