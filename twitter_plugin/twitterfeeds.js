...
$(document).ready(function(){
    ...
    $.get('ajax_sponsors.php',function(response){
     ...
     var defaulthtml = '';
    defaulthtml += ' <p style="width:100%;"  > <span   ><a href="https://twitter.com/Gazal_Sathiya/status/4586772030" target="_blank"><img style="border-radius:26px;" src="https://abs.twimg.com/sticky/default_profile_images/default_profile_1_bigger.png">'+ urltag('@Gazal_Sathiya') +'</a> </span><span >'+  urltag('Spreading innovation, awareness, integrity through creativity arts and drawings!') + ' <br/><span class="created_at text-info">' + xTimeAgo(1462712683) + '</span></span></p>';
    defaulthtml += '<br/><hr />';
    $('#tweecool').hide().prepend(defaulthtml).fadeIn(500);
    setTimeout(function(){
        // sponsortweets('@Gazal_Sathiya','1');
...
/* ---------------------------------- */
function sponsortweets(screenname,count){
    $.ajax({
        url: 'twitter_feeds.php?sponsor_name=' + screenname,
        crossDomain: true,
        dataType: 'json'
    }).done(function (data) {
        var html = '';
        // $.each(data.tweets, function(i, item) {
            html += ' <p style="width:100%;"  > <span  ><a href="https://twitter.com/' + screenname +'" target="_blank"><img style="border-radius:26px;" src="' + data.tweets.img + '">'+ screenname +'</a> </span><span >'+  urltag(data.tweets.text) + ' <span class="created_at">' + xTimeAgo(data.tweets.timestamp) + '</span></span></p>';
            html += '<br/><hr />';
        // });
