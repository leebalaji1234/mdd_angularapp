...
if ($app->is_post() || $doPostLogin) {
  ...
  if (empty($_REQUEST['email']))
    ...
    if ($user->_new_row || @base64_encode($_REQUEST['pass']) != $user->pass)
    ...
    elseif ($user->status != 1)
      $app->error('##Your User ID is blocked. Contact admin.##');  
    else {
      ...
      if(isset($_REQUEST['rememberme'])){
        ...
        setcookie("password", base64_encode($_REQUEST['pass']), time() + (365 * 24 * (60 * 60)));  
      }
