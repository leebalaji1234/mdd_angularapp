...
$tblv = new Volunteer;
$alldrawings = $tbldrawings->find_all('where status = 0 and reports IS NULL ORDER BY likes DESC, (clicks + views) DESC LIMIT 10');
// print_r($alldrawings);exit;
 $tblp = new Payment;
 $tblr = new Region;
$res =& $db->query("select r.id,r.email,r.title,r.url from mp_payments as p INNER JOIN mp_regions as r ON (p.region_id = r.id) where r.status = 1 group by p.region_id order by p.amount desc limit 10");
...
$res1 = & $db->query('SELECT v . * , COUNT( d.volunteer_id ) AS rank
FROM mp_volunteers AS v
INNER JOIN mp_drawings AS d ON ( d.volunteer_id = v.id ) 
GROUP BY d.volunteer_id
HAVING COUNT( rank ) >=10 ');
$allvolunteers = [];
  while ($row1 =& $res1->fetchRow(DB_FETCHMODE_OBJECT)) {
     $allvolunteers[] = $row1;
 }
